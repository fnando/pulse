...
test("increment/decrement", async ({ page }) => {
  await page.goto(`file://${__dirname}/counter.html`);
...
test("increment/decrement (iife)", async ({ page }) => {
  await page.goto(`file://${__dirname}/iife.html`);
  await page.getByRole("button", { name: "Increment" }).click();
  await expect(page.locator("strong")).toHaveText("1");
  await page.getByRole("button", { name: "Decrement" }).click();
  await expect(page.locator("strong")).toHaveText("0");
...
test("delayed component being added to DOM", async ({ page }) => {
  await page.goto(`file://${__dirname}/delayed.html`);
  await page.getByRole("button", { name: "Increment" }).click();
  await expect(page.locator("strong")).toHaveText("1");
  await page.getByRole("button", { name: "Decrement" }).click();
  await expect(page.locator("strong")).toHaveText("0");
...
test("multiple controllers", async ({ page }) => {
  await page.goto(`file://${__dirname}/multiple-controllers.html`);
...
test("multiple controllers on the same element", async ({ page }) => {
  ...
  await page.goto(`file://${__dirname}/multiple-controllers-same-element.html`);
  await expect(
    page.locator(
...
test("event options", async ({ page }) => {
  await page.goto(`file://${__dirname}/event-options.html`);
...
test("using @window", async ({ page }) => {
  await page.goto(`file://${__dirname}/window-events.html`);
...
test("using keyboard events", async ({ page }) => {
  ...
  await page.goto(`file://${__dirname}/keyboard-events.html`);
